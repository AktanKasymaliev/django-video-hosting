...
class VideoStream(BaseStream):
    ...
    def ranged(self,
        ...
        block_size: int = 8192, # bytes for returne partly
    ) -> Generator[bytes, None, None]:
        consumed = 0 # start point for video
        
        # Read video file in bytes
        file.seek(start)
        while True:
    ...
    def open_file(self, request, video_pk: int) -> tuple:
        ...
        #Get path of videofile
        path = Path(_video.file.path)

        # Read videofile and take all size of video in bytes
        file = path.open('rb')
        file_size = path.stat().st_size

        # find video length and video during range from headers
        content_length = file_size
        status_code = 200
        content_range = request.headers.get('range')

        # Find exactly video length in bytes  
